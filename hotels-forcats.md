...
``` r
hotels %>%group_by(hotel, arrival_date_month) %>%# group by hotel type and arrival month
 ...
 scale_y_continuous(labels=scales::dollar_format())+
  ...
  labs(x = "Arrival month",                 # customize labels
       ...
       subtitle = "Resort hotel prices soar in the summer while ciry hotel prices remain relatively constant throughout the year",color = "Hotel type")
```
